...
This script ONLY plots — all model/training/forecast logic lives in the library. Two modes:
  ...
  * --ckpt PATH: load a saved checkpoint (from train_action_dynamics.py) and plot that one model.
    ...
    python scripts/actionsense/plot_action_forecast.py --actions Slice,Peel --viz-action Slice --target 0
    python scripts/actionsense/plot_action_forecast.py --ckpt runs/ad_slice-peel.pt --viz-action Slice
"""
