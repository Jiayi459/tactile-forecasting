...
\begin{figure*}[t]
    ...
    \caption{Study logic, drawn from ActionSense held-out data. \textbf{(1)} Temporal structure determines how strong persistence is: two real $F$ traces with their measured $\Rdiff$, and $\Rdiff$ for all 290 scored recordings. Curves are labelled by measured temporal structure, not by the prespecified smooth/abrupt class, whose two distributions overlap on this axis. \textbf{(2)} Each map becomes one physical state $\mathbf{s}_t=[F,c^x,c^y]$; rolling origins advance by one sample, and only tactile history precedes the origin. \textbf{(3)} One held-out origin (Seq2Seq, \emph{peel}): the same forecast is read as absolute future-state accuracy, as pointwise gain beyond persistence, and as trajectory-shape error, so that baseline difficulty is not misread as learned forecasting ability. Here the forecast beats copy-last pointwise ($S=+.48$) while its worst point still lies $2.0$ target standard deviations from the true trajectory. Panel (3) is a single-origin illustration; population results are in Tables~\ref{tab:opentouch_results}--\ref{tab:actionsense_results}.}
    \label{fig:overview}
\end{figure*}
