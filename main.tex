...
\begin{figure*}[t]
    \centering
    % Generated by scripts/shared/plot_study_logic.py from runs/as_preds_seq2seq_corpus_h1
    % (the 1 s-history Seq2Seq run), clip 115, origin t=103. Nothing is synthetic.
    ...
    \caption{The forecasting task and the skill ratio, on one held-out ActionSense origin (Seq2Seq, 1~s history, \emph{peel}; total force only, arbitrary units). One second of force history is the input; the next second is predicted. Over the predicted second the ground truth, the forecast and the persistence baseline---which repeats the last observed value---are drawn together, and at every horizon step the two errors that form the skill are marked: $e_{\mathrm{pers}}$ from truth to persistence and $e_{\mathrm{model}}$ from truth to forecast. Skill is one read against the other, so a forecast is credited only for the error it removes from copy-last, not for how accurate it looks. The figure writes the ratio in its simplest form; the reported quantity squares and averages both errors over the horizon, $S_{\mathrm{pers}}=1-\mathrm{MSE_{model}}/\mathrm{MSE_{persistence}}$ \eqref{eq:skill}. The origin shown is an illustration of the definition; the reported $S$ averages over every origin and weights recordings equally (Tables~\ref{tab:opentouch_results}--\ref{tab:actionsense_results}).}
    \label{fig:overview}
\end{figure*}
