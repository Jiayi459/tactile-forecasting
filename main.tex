...
\begin{figure*}[t]
    \centering
    % Generated by scripts/shared/plot_study_logic.py from the saved corpus forecasts
    % (runs/as_preds_seq2seq_corpus) and data/actionsense_states; nothing is synthetic.
    \IfFileExists{figures/overview.pdf}{%
        \includegraphics[width=\textwidth]{figures/overview.pdf}%
    }{\placeholderfigure{1.00in}{Run \texttt{python scripts/shared/plot\_study\_logic.py} and upload \texttt{figures/overview.pdf}.}}
    \caption{Study logic. Panels use ActionSense held-out data; the OpenTouch marker in panel~1 is the frozen cross-sensor $\Rdiff$ table. \textbf{(1)} Temporal structure determines how strong persistence is. Two real $F$ traces with their measured $\Rdiff$, then $\Rdiff$ for each of the 290 scored ActionSense recordings and, on the same axis, the OpenTouch corpus mean. $\Rdiff$ is a continuum spanning both corpora, and OpenTouch sits near the decorrelated end, so identical scores on the two corpora do not describe equally hard problems. Traces are labelled by measured temporal structure, not by the prespecified smooth/abrupt class, whose ActionSense distributions overlap here; the preregistered class contrast is tested on OpenTouch (Sec.~\ref{sec:results_actions}). \textbf{(2)} Each map becomes one physical state $\mathbf{s}_t=[F,c^x,c^y]$; rolling origins advance by one sample, and only tactile history precedes the origin. \textbf{(3)} One held-out origin (Seq2Seq, \emph{peel}): the same forecast is read as absolute future-state accuracy, as pointwise gain beyond persistence, and as trajectory-shape error, so that baseline difficulty is not misread as learned forecasting ability. Here it beats copy-last pointwise ($S=+.48$) while its worst point still lies $2.0$ target standard deviations from the true trajectory. Panel~(3) is a single-origin illustration; population results are in Tables~\ref{tab:opentouch_results}--\ref{tab:actionsense_results}.}
    \label{fig:overview}
\end{figure*}
